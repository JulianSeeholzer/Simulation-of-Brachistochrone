...
pygame_icon = pygame.image.load('Image_8.png')
...
tkscreen.configure(bg='black')  # define background color of the tkinter window
os.environ['SDL_VIDEODRIVER'] = 'windib'  # get access of operating system parameters
tkinter_icon = PhotoImage(file="Image_8.png")
...
def dynamic_starting_ball1(space):  # define a function for a dynamical body sitting on the top of the inclined line
    ...
    body.position = (h[0][0]+r/2, h[0][1]-r)  # define position of the body
...
def dynamic_starting_ball2(space):  # define a function for a dynamical body sitting on the top of the Brachistochrone curve
    r = float(entry_r.get())  # define variable for radius of the pymunk body
    body = pymunk.Body(1, 0.01, body_type=pymunk.Body.DYNAMIC)  # create dynamic pymunk body
    body.position = (p[0][0]+r/2, p[0][1]-r)  # define position of the body
    shape = pymunk.Circle(body, r)  # create shape for body with radius r
    space.add(body, shape)  # add body and shape to pymunk space
    return shape
...
scale_g = tk.Scale(tkscreen, bg='black', fg='white', from_=0, to=100, orient=tk.HORIZONTAL, length=200, bd=10)  # create a tkinter scale
scale_g.config(highlightbackground='black')  # define border color
scale_g.pack()  # add the scale to the tkinter window
scale_g.place(x=107, y=387)  # define absolute position of the scale

label_g_off_image = PhotoImage(file='Image_5.png')  # define an image for button
label_g_off = Button(tkscreen, bg='black', image=label_g_off_image, bd=0, command=gravity_off)  # create a tkinter button
...
label_g_on_image = PhotoImage(file='Image_4.png')  # define an image for button
label_g_on = Button(tkscreen, bg='black', image=label_g_on_image, bd=0, command=gravity_on)  # create a tkinter button
label_g_on.pack()  # add the button to the tkinter window
label_g_on.place(x=380, y=410)  # define absolute position of the button

label_startingballs_image = PhotoImage(file='Image_6.png')  # define an image for button
label_startingballs = Button(tkscreen, bg='black', image=label_startingballs_image, bd=0, command=dynamic_starting_balls)  # create a tkinter button
...
label_g_strength_image = PhotoImage(file='Image_3.png')  # define an image for label
label_g_strength = tk.Label(tkscreen, bg='black', image=label_g_strength_image)  # create a tkinter label
...
label_g_image = PhotoImage(file='Image_1.png')  # define an image for label
label_g = tk.Label(tkscreen, bg='black', image=label_g_image)  # create a tkinter label
label_g.pack()   # add label to the tkinter window
label_g.place(x=70, y=230)  # define absolute position of the label

label_r_image = PhotoImage(file='Image_2.png')  # define an image for label
label_r = tk.Label(tkscreen, bg='black', image=label_r_image)  # create a tkinter label
label_r.pack()   # add label to the tkinter window
label_r.place(x=70, y=660)  # define absolute position of the label

label_title_image = PhotoImage(file='Image_7.png')  # define an image for label
label_title = tk.Label(tkscreen, bg='black', image=label_title_image)  # create a tkinter label
...
while True:  # game loop
    ...
    visualize_dynamic_ball(balls)  # call function to draw the dynamic bodys
    visualize_dynamic_startingballs(startingballs)  # call function to draw the dynamic bodys at the top of the curves
